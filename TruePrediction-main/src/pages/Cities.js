...
export default function Cities() {
    ...
    useEffect(() => {
        axios.get("http://localhost:8080/cities").then(res => setPreview(res.data));
    }, []);
        ...
        <main>
            ...
            <div>
                ...
                {preview && preview.map((p, index) => {
                        return (
                            <div id="preview" key={index}>
                                <div>
                                    <div>
                                        <h1>{p.nameCity}</h1>
                                        <div style={{ display: "flex", alignItems: "center" }}>
                                            <h1>{(p.temperature.$numberDecimal - 273.15).toFixed(2)}°</h1>
                                            {p.weatherId === 804 && (
                                            ...
                                            {(p.weatherId === 803 || p.weatherId === 802 || p.weatherId === 801) && (
                                        ...
                                        <h2>Sensação térmica: {(p.feels_like.$numberDecimal - 273.15).toFixed(2)}°</h2>
                                        <p>Consultado em: {new Date(p.createdAt).toLocaleDateString()}</p>
                                    </div>
                                    <div>
                                        <p>Temperatura máxima: {(p.temperature_max.$numberDecimal - 273.15).toFixed(2)}°</p>
                                        <p>Temperatura mínima: {(p.temperature_min.$numberDecimal - 273.15).toFixed(2)}°</p>
                                        <p>Pressão: {p.pressure.$numberDecimal}hPa</p>
                                        <p>Umidade: {p.humidity}%</p>
                                        <p>Velocidade do vento: {(p.windSpeed.$numberDecimal * 3.6).toFixed(2)}km/h</p>
                                        <p>Precipitação: {p.pop ? 100 : 0}%</p>
                                    </div>
                                </div>
                                <br />
                            </div>
