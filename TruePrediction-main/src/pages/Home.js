...
function Home() {
    ...
    const previewCity = (lat, lng) => {
        setResult([]);
        axios.get(`http://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lng}&appid=cd167360712f9406b8262a06d1b7f628`).then(preview => {
            setPreview(preview.data);

            setUserData({
                labels: preview.data.list.map(day => getDate(day.dt_txt)),
                datasets: [
                    {
                        label: "Temperatura",
                        data: preview.data.list.map(p => (p.main.feels_like - 273.15).toFixed(2)),
